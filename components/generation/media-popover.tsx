...
import {
  ensureVoicesLoaded,
  pickPreferredBrowserVoiceId,
  toBrowserTTSVoiceInfo,
...
export function MediaPopover({ onSettingsOpen }: MediaPopoverProps) {
  ...
  useEffect(() => {
    if (ttsProviderId !== 'browser-native-tts' || typeof window === 'undefined') {
      setBrowserVoices([]);
      return;
    }
    ...
    const loadBrowserVoices = async () => {
      ...
      const mapped = toBrowserTTSVoiceInfo(loadedVoices);
      setBrowserVoices(mapped);

      if (mapped.length === 0) return;

      const hasCurrentVoice = mapped.some((v) => v.id === ttsVoice);
      if (!ttsVoice || ttsVoice === 'default' || !hasCurrentVoice) {
        const preferredVoice =
          pickPreferredBrowserVoiceId(loadedVoices, 'female', locale) || mapped[0].id;
  ...
  }, [ttsProviderId, ttsVoice, locale, setTTSVoice]);
  ...
  const ttsVoices = useMemo(
    () =>
      (ttsProviderId === 'browser-native-tts' && browserVoices.length > 0
        ? browserVoices
        : getTTSVoices(ttsProviderId)
      ).map((v) => ({
        id: v.id,
        name: `${getVoiceDisplayName(v.name, locale)} ${getGenderLabel(v.gender, locale)}`.trim(),
      })),
    [ttsProviderId, browserVoices, locale],
  );
