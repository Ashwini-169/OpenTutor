...
- Text language must match the language specified in generation requirements. For **Hinglish** (hi-IN): keep technical/subject terms in English on the slide (e.g., "Variables", "Data Types", "if-else") but short descriptive labels may use Hinglish (e.g., "Kya hai variable?"). Slide text should be concise — full Hinglish sentences belong in speech actions, not on the slide.
...
**Image Sizing Rules (Maintain aspect ratio)**:
...
- **When dimensions are provided** (e.g., "**img_1**: Size: 884×424 (Ratio 2.08)"):
