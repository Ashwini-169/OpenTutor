...
interface StageState {
  ...
  prioritySlideQueue: string[];
  prioritySpeechQueue: string[];
  speechFailedSceneIds: string[];
  ...
  addPrioritySlide: (outlineId: string) => void;
  removePrioritySlide: (outlineId: string) => void;
  addPrioritySpeech: (sceneId: string) => void;
  removePrioritySpeech: (sceneId: string) => void;
  addSpeechFailedScene: (sceneId: string) => void;
  removeSpeechFailedScene: (sceneId: string) => void;
...
const useStageStoreBase = create<StageState>()((set, get) => ({
  ...
  addPrioritySlide: (outlineId) => {
    const existed = get().prioritySlideQueue.includes(outlineId);
    if (!existed) {
      set({ prioritySlideQueue: [...get().prioritySlideQueue, outlineId] });
    }
  },

  removePrioritySlide: (outlineId) => {
    set({
      prioritySlideQueue: get().prioritySlideQueue.filter((id) => id !== outlineId),
    });
  },

  addPrioritySpeech: (sceneId) => {
    const existed = get().prioritySpeechQueue.includes(sceneId);
    if (!existed) {
      set({ prioritySpeechQueue: [...get().prioritySpeechQueue, sceneId] });
    }
  },

  removePrioritySpeech: (sceneId) => {
    set({
      prioritySpeechQueue: get().prioritySpeechQueue.filter((id) => id !== sceneId),
    });
  },

  addSpeechFailedScene: (sceneId) => {
    const existed = get().speechFailedSceneIds.includes(sceneId);
    if (!existed) {
      set({ speechFailedSceneIds: [...get().speechFailedSceneIds, sceneId] });
    }
  },

  removeSpeechFailedScene: (sceneId) => {
    set({
      speechFailedSceneIds: get().speechFailedSceneIds.filter((id) => id !== sceneId),
    });
  },
