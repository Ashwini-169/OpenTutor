...
export function GenerationToolbar({
  ...
  const configuredProviders = providersConfig
    ? Object.entries(providersConfig)
        ...
        .filter((provider) => provider.models.length >= 1)
    : [];
            ...
            {configuredProviders.map((provider) => {
              ...
              return (
                <button
                  key={provider.id}
                  onClick={() => {
                    // Fast-path: if this provider has only one available model,
                    // selecting the provider should immediately apply that model.
                    if (provider.models.length === 1) {
                      setModel(provider.id, provider.models[0].id);
                      setPopoverOpen(false);
                      return;
                    }
                    setDrillProvider(provider.id);
                  }}
                  className={cn(
