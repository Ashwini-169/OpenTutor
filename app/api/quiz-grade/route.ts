...
export async function POST(req: NextRequest) {
  try {
    ...
    const systemPrompt = isZh
      ? `Aap ek professional education assessment expert hain. Please question aur student ke answer ke basis par score karein aur ek short comment dein.
Aapko sirf is JSON format mein reply dena hai (kuch aur mat likhna):
{"score": <0 se ${points} tak ka integer>, "comment": "<ek ya do lines ka feedback>"}`
...
{"score": <integer from 0 to ${points}>, "comment": "<one or two sentences of feedback>"}`;
    ...
    const userPrompt = isZh
      ? `Sawaal: ${question}
Maximum Marks: ${points} marks
${commentPrompt ? `Grading Tips: ${commentPrompt}\n` : ''}Student ka answer: ${userAnswer}`
      : `Question: ${question}
...
${commentPrompt ? `Grading guidance: ${commentPrompt}\n` : ''}Student answer: ${userAnswer}`;
    ...
    try {
      ...
      gradeResult = {
        ...
        comment: isZh
          ? 'Answer submit ho gaya hai, please standard answer check karein.'
