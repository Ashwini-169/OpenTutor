...
interface ProviderConfigPanelProps {
  ...
  onAddModel: () => void;
  onDetectOllamaModels?: () => Promise<void>;
...
export function ProviderConfigPanel({
  ...
  const isOllamaProvider = provider.id === 'ollama';

  const handleDetectOllamaModels = useCallback(async () => {
    if (!onDetectOllamaModels) return;
    setDetectingOllama(true);
    try {
      await onDetectOllamaModels();
    } finally {
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isOllamaProvider && onDetectOllamaModels && (
              <Button
                variant="outline"
                size="sm"
