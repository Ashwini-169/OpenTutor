...
export function SceneSidebar({
  ...
  const speechFailedSceneIds = useStageStore.use.speechFailedSceneIds();
  ...
  const [retryingSpeechSceneId, setRetryingSpeechSceneId] = useState<string | null>(null);
  
  const handleRetrySpeech = async (e: React.MouseEvent, sceneId: string) => {
    e.stopPropagation();
    if (!onRetrySpeech) return;
    setRetryingSpeechSceneId(sceneId);
    try {
      await onRetrySpeech(sceneId);
    } finally {
      setRetryingSpeechSceneId(null);
    }
  };
