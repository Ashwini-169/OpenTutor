...
async function directorNode(
  ...
  try {
    ...
    let nextAgentId = decision.nextAgentId;
    let agentExists = agents.some((a) => a.id === nextAgentId);
    
    if (!agentExists && nextAgentId && nextAgentId !== 'USER') {
      // Try to fallback match by name if the LLM hallucinated the agent name instead of the ID
      const matchedByName = agents.find((a) => a.name.toLowerCase() === nextAgentId!.toLowerCase());
      if (matchedByName) {
        log.info(`[Director] Resolved agent name "${nextAgentId}" to ID "${matchedByName.id}"`);
        nextAgentId = matchedByName.id;
        agentExists = true;
      }
    }
    ...
    log.info(`[Director] Decision: dispatch agent "${nextAgentId}"`);
    return {
