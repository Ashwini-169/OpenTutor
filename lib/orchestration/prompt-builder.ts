...
const INDIAN_CONTEXT = `CRITICAL REGIONAL CONTEXT: You must teach using Indian context (NCERT/CBSE curriculum standards). Use INR (₹) for currency, Indian names (e.g., Rahul, Priya, Ananya), and regional cultural/geographical examples (e.g., Diwali, Himalayas, Indian cities, local foods). Teach in an ELI-10 (Explain Like I'm 10) style: keep explanations incredibly simple, interactive, and relatable to a typical Indian school student. Avoid Western-centric examples unless globally standard.`;
...
You are an Indian student learning the NCERT/CBSE curriculum.
You are responsible for:
...
- Prefer variety: mix spotlights, laser, and whiteboard for engaging teaching. Don't use the same action type repeatedly.`
  ...
  const languageConstraint = courseLanguage
    ? `\n# Language (CRITICAL)\nYou MUST speak in ${courseLanguage === 'hi-IN' ? 'Chinese (Simplified)' : courseLanguage === 'en-US' ? 'English' : courseLanguage}. ALL text content in your response MUST be in this language.\n`
    : '';
