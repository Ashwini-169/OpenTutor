    ...
    <div className="w-full h-full flex flex-col bg-gray-50 dark:bg-gray-900 group/canvas">
      ...
      >
        ...
        >
          ...
          {/* Fallback Speech Failed UI */}
          <AnimatePresence>
            {isSpeechFailed && currentScene && !isPendingScene && (
              <motion.div
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.95 }}
                className="absolute bottom-4 right-4 z-[106] bg-amber-50 dark:bg-amber-900/80 border border-amber-200 dark:border-amber-700/50 rounded-lg p-3 shadow-lg flex items-center gap-3 backdrop-blur-md"
              >
                ...
                <div className="flex flex-col">
                  <span className="text-sm font-semibold text-amber-800 dark:text-amber-200">
                    {t('generation.speechFailedBanner')}
                  </span>
                  <span className="text-xs text-amber-700 dark:text-amber-300">
                    {t('generation.regenerateSpeechDesc')}
                  </span>
                </div>
                {onRetrySpeech && (
                  <button
                    ...
                    className="ml-2 px-3 py-1.5 text-xs font-medium rounded border border-amber-300 dark:border-amber-600 bg-white dark:bg-amber-800 hover:bg-amber-100 dark:hover:bg-amber-700 text-amber-700 dark:text-amber-100 transition-colors shadow-sm whitespace-nowrap"
                  >
                    {t('generation.regenerateSpeech')}
                  </button>
                )}
              </motion.div>
            )}
