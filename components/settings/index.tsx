...
export function SettingsDialog({ open, onOpenChange, initialSection }: SettingsDialogProps) {
  ...
  const handleDetectOllamaModels = useCallback(async () => {
    const config = providersConfig.ollama;
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        toast.error(data.error || 'Failed to detect Ollama models');
        return;
      }

      const detectedModels = Array.isArray(data.models) ? data.models : [];
      if (detectedModels.length === 0) {
        toast.error('No Ollama models detected');
        return;
      }

      setProviderConfig('ollama', { models: detectedModels });
      toast.success(`Detected ${detectedModels.length} Ollama model(s)`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to detect Ollama models');
    }
            ...
            <div className="flex-1 overflow-y-auto p-5">
              ...
              {activeSection === 'providers' && selectedProvider && (
                <ProviderConfigPanel
                  ...
                  onDetectOllamaModels={handleDetectOllamaModels}
