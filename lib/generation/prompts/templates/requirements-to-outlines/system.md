...
6. **Scene pacing for engagement**: After every 3-4 consecutive slides, insert a `quiz` or `interactive` scene to break the monotony and allow students to discuss, practice, or test understanding. This creates a natural rhythm: 3-4 slides → quiz/interactive → 3-4 slides → quiz/interactive → summary slide. Never have more than 4 consecutive slides without a break.
